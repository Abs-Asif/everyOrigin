...
const googleSansStack = `"Google Sans", ${openSans.style.fontFamily}, sans-serif`;
...
export default function ArticleOrigin() {
  ...
  }/v2/get?url=${encodeURIComponent(url)}");\nconst result = await response.json();`;
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setLoading(true);
      const response = await fetch(`/api/v2/get?url=${encodeURIComponent(validUrl.toString())}`);
  ...
  return (
    <main
      className={`flex min-h-screen flex-col items-center justify-between gap-6 p-4 pt-10 text-neutral-900 sm:gap-12 sm:p-12 md:p-24 dark:text-neutral-100 ${openSans.className}`}
    >
      <Head>
        <title>ARTICLEOrigin</title>
        <meta
          name="description"
          content="ARTICLEOrigin is a free CORS proxy for articles. Get title and full description without CORS issues."
        />
      ...
      <div className="relative m-auto flex flex-col items-center after:absolute after:top-0 after:-z-20 after:h-[180px] after:w-[180px] after:animate-[pulse_10s_ease-in-out_infinite] after:bg-gradient-conic after:from-sky-200 after:via-blue-200 after:blur-2xl after:content-[''] after:sm:w-[360px] before:lg:h-[360px] before:dark:bg-gradient-to-br before:dark:from-transparent before:dark:to-blue-700/10 after:dark:from-sky-900 after:dark:via-[#0141ff]/40">
        <h1 className="text-center text-4xl font-extrabold tracking-tight sm:text-6xl" style={{ overflowWrap: "anywhere", fontFamily: googleSansStack }}>
          ARTICLEOrigin
        </h1>
        <h2 className="mt-2 text-center text-lg font-medium opacity-80 sm:text-2xl md:text-3xl" style={{ fontFamily: googleSansStack }}>
          Free CORS proxy for articles
        </h2>
      </div>
          ...
          <div className="p-4 sm:p-6">
            ...
            <div className="mt-3 flex items-center justify-between">
              <p className="text-xs text-neutral-500 dark:text-neutral-400">
                Tip: Enter any news article URL.
              </p>
              {fetchTime && (
            ...
            <div className="relative overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-xl dark:border-neutral-800 dark:bg-neutral-900">
              ...
              <div className="flex flex-col p-6">
                <div className="flex flex-col justify-center">
                  ...
                  <h3 className={`mb-4 text-3xl font-bold leading-tight`} style={{ fontFamily: googleSansStack }}>{metadata.title || "No title found"}</h3>
                  {metadata.article && (
                    <div className={`text-base text-neutral-700 dark:text-neutral-300 whitespace-pre-wrap`} style={{ fontFamily: googleSansStack }}>
                      {metadata.article}
                    </div>
                  )}
                  {!metadata.article && metadata.description && (
                    <p className={`text-base text-neutral-600 dark:text-neutral-400`} style={{ fontFamily: googleSansStack }}>
                      {metadata.description}
                    </p>
                  )}
                  <p className="mt-6 text-xs text-neutral-500 dark:text-neutral-500 italic break-all">
      ...
      <div className="w-full max-w-4xl font-sans">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-2 sm:gap-6">
          <div className="rounded-xl border border-neutral-200 bg-white p-5 sm:p-6 shadow-md dark:border-neutral-800 dark:bg-neutral-900">
            <h3 className="mb-3 text-xl font-bold text-blue-600 dark:text-blue-400">1. Article Mode</h3>
            <p className="mb-4 text-sm text-neutral-600 dark:text-neutral-400">
              ARTICLEOrigin extracts the full article content from web pages, returning it along with the title and description in a JSON object.
            </p>
          ...
          <div className="rounded-xl border border-neutral-200 bg-white p-5 sm:p-6 shadow-md dark:border-neutral-800 dark:bg-neutral-900">
            <h3 className="mb-3 text-xl font-bold text-blue-600 dark:text-blue-400">2. Image Redirection</h3>
            <p className="mb-4 text-sm text-neutral-600 dark:text-neutral-400">
              If the URL points to an image, ARTICLEOrigin redirects the request to the main NEWSOrigin API.
            </p>
            <div className="rounded bg-neutral-100 p-2 text-xs font-mono dark:bg-neutral-800">
              GET /v2/get?url=example.com/image.png
              <br/>
              &rarr; Redirects to /get?url=...
            </div>
          </div>
        </div>
      </div>
